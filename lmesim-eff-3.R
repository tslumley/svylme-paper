...
one.simeff3<-function(){
pop<-data.frame(x=rnorm(10000)+rep(rnorm(1000),each=10))
pop$z<-rgamma(10000,2)
pop$id<-1:10000
pop$g<-rep(1:1000,each=10)
pop$j<-rep(1:10, 1000)
pop$strat<-rep(1:10,each=1000)
pop$u<-rnorm(1000,s=.2)[pop$g]
pop$v<-rnorm(1000)[pop$g]+pop$u
pop$y<-with(pop, 
	x*5+z+rnorm(10000,s=1.5)+rnorm(1000,s=.4)[g]+ u*z)
